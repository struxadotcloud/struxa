          ...
          {eggs?.map((egg) => (
            ...
            >
              <div className="flex items-center gap-4">
                ...
                {egg.description && (
                  <span className="max-w-md truncate text-xs text-[#555555]">{egg.description}</span>
                )}
                <span className="border border-[#333333] px-1.5 py-0.5 text-[10px] uppercase tracking-wider text-[#444444]">
                  {egg.variables.length} vars
                </span>
              </div>
              <div className="flex items-center gap-3">
                <span className="max-w-xs truncate font-mono text-xs text-[#444444]">
                  {egg.startup}
                </span>
                <Link
                ...
                {confirmDelete === egg.id ? (
                  <div className="flex items-center gap-2">
                    <button
                      type="button"
                      onClick={async () => {
                        await deleteMutation.mutateAsync({ id: egg.id });
