  ...
  return (
    <div className={`${dims} shrink-0 overflow-hidden ${rounded} bg-muted flex items-center justify-center`}>
      {mod.icon_url ? (
        ...
        <Box className={`${iconDims} text-muted-foreground`} />
      )}
    </div>
  );
}
...
function ModCard({ mod, onClick }: { mod: ModrinthProject; onClick: () => void }) {
  ...
  return (
    ...
    >
      <div className="flex items-start gap-3">
        <ModIcon mod={mod} size="md" />
        <div className="flex min-w-0 flex-1 flex-col gap-0.5 pt-0.5">
          <p className="truncate text-sm font-semibold text-foreground leading-tight">{mod.title}</p>
          <p className="truncate text-xs text-muted-foreground">{t("author", { author: mod.author })}</p>
        </div>
      ...
      <p className="line-clamp-2 text-xs leading-relaxed text-muted-foreground">{mod.description}</p>
    ...
    fetch(`${MODRINTH_API}/project/${projectId}/version`)
      ...
      .then((all) => {
        const compatible = all.filter(
          (v) => v.loaders.length === 0 || v.loaders.some((l) => MOD_LOADERS.includes(l)),
        );
        ...
        for (const v of pool)
          for (const l of v.loaders)
            if (MOD_LOADERS.includes(l)) counts.set(l, (counts.get(l) ?? 0) + 1);
...
function ModDetail({
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch(`${MODRINTH_API}/project/${mod.project_id}`).then((r) => r.json() as Promise<ModrinthProjectDetail & { body: string }>),
      fetch(`${MODRINTH_API}/project/${mod.project_id}/members`).then((r) => r.json() as Promise<ModrinthMember[]>),
    ])
      .then(([proj, mems]) => {
          ...
          <div className="flex w-72 shrink-0 flex-col gap-5 overflow-y-auto border-r border-border p-6">
            <div className="flex items-start gap-3 pr-6">
              ...
              <div className="flex min-w-0 flex-col gap-1 pt-1">
                <p className="font-semibold text-foreground leading-tight">{mod.title}</p>
                <p className="text-xs text-muted-foreground">{t("author", { author: mod.author })}</p>
            ...
            <VersionPicker projectId={mod.project_id} onSelect={setSelectedVersion} />
  ...
  return (
    <>
        ...
        <SheetPopup side="bottom" showCloseButton>
          <SheetHeader>
            <div className="flex items-center gap-3 pr-8">
              <ModIcon mod={mod} size="sm" />
              <div className="flex min-w-0 flex-col gap-0.5">
                <SheetTitle className="truncate text-base">{mod.title}</SheetTitle>
                <p className="text-xs text-muted-foreground">{t("author", { author: mod.author })} · <Download className="inline h-2.5 w-2.5" /> {fmtDownloads(mod.downloads)}</p>
              </div>
            </div>
          </SheetHeader>
      ...
      <Sheet open={installSheetOpen} onOpenChange={setInstallSheetOpen}>
        <SheetPopup side="bottom" showCloseButton>
          <SheetHeader>
            <SheetTitle>{t("install")} {mod.title}</SheetTitle>
          </SheetHeader>
...
export default function ModsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const unsupported = eggFeatures !== null && !eggFeatures.includes("minecraft_mods");
        ...
        <div className="flex-1 overflow-y-auto p-4">
          ...
          {searchLoading ? (
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
              {Array.from({ length: 12 }).map((_, i) => <ModCardSkeleton key={i} />)}
            </div>
          ) : results && results.length > 0 ? (
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
              {results.map((mod) => (
                <ModCard key={mod.project_id} mod={mod} onClick={() => setSelectedMod(mod)} />
              ))}
            </div>
          ) : (
